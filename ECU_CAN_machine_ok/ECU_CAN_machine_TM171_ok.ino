...
//11/2025 FlorianT ajout module machine
// ajout du sectioncontrol.cpp et .h, quelque ligne dans ce code reperé par FlorianT
//----------------------------------------------------------
...
String inoVersion = ("\r\nAgOpenGPS Tony UDP CANBUS Ver 04.05.2024");
  ...
  #include "SectionControl.h" //FlorianT
...
char rxbuffer[serial_buffer_size];         //Extra serial rx buffer
char txbuffer[serial_buffer_size];         //Extra serial tx buffer
...
//*******************************************************************************
  ...
  Serial.println(useTM171);
    ...
    {           
      ...
      EEPROM.put(96, aogConfig); //Machine modif FlorianT
    ...
    { 
      ...
      EEPROM.get(96, aogConfig); //Machine modif FlorianT
    }
    ...
    //----Teensy 4.1 Ethernet--Start---------------------
      ...
      GPS->addMemoryForRead(rxbuffer, serial_buffer_size);
      GPS->addMemoryForWrite(txbuffer, serial_buffer_size);
...
// //Bit 10 Tram  commenter jusqu'a udpData 12 (FlorianT)
// tram = udpData[10];
...
// relayHi = udpData[12];
  ...
  else if (udpData[3] == 236) // Relay Pin Settings ajouté par FlorianT
{
    updatePinMapping(udpData);
}
...
sendHelloToAgIO(); //  ajouté par FlorianT
    }


   else if (udpData[3] == 0xEF)  //239 Machine Data FlorianT
    {
        handlePGN239(udpData);  // Tout est géré dans SectionControl.cpp
    ...
    {         
      ...
      EEPROM.put(96, aogConfig); // Modif FlorianT
