...
void handlePGN239(uint8_t* autoSteerUdpData);
...
void sendSectionStatus()
{
    ...
    Eth_udpAutoSteer.beginPacket(ipDestination, AOGPort);
    Eth_udpAutoSteer.write(PGN_237, sizeof(PGN_237));
...
void sendManualAutoStatus()
{
    ...
    // Envoyer à AgOpenGPS
    Eth_udpAutoSteer.beginPacket(ipDestination, AOGPort);
    Eth_udpAutoSteer.write(PGN_234, sizeof(PGN_234));
    Eth_udpAutoSteer.endPacket();
}
...
void handlePGN239(uint8_t* autoSteerUdpData)
{
    ...
    gpsSpeed = (float)autoSteerUdpData[6];
    hydLift = autoSteerUdpData[7];
    tram = autoSteerUdpData[8];  
